// 获取默认配置
const getDefaultOptions = scopes => {
  // 自定义 scope，提供给lint校验区域
  const scopesLint = scopes.map(scope => {
...
const defineConfig = (
  ...
  const scopes = options.prompt.scopes || []
  // 默认配置
  const defaultOptions = getDefaultOptions(scopes)
  return {
    rules: {
